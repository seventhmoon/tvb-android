...
import com.tvb.android.DetailsActivity;
import com.tvb.android.PlayListHelper;
import com.tvb.android.R;
import com.tvb.android.Utils;
import com.tvb.android.VideoChannel;

import java.util.Timer;
import java.util.TimerTask;

public class TvbPlayerActivity extends Activity {
    ...
    private VideoChannel mSelectedVideoChannel;
...
//        startVideoPlayer();
        ...
        new SetupPlaylistTask().execute();
        updateMetadata(true);
    }

    private void setupVideoChannel() {
        Bundle b = getIntent().getExtras();
        mSelectedVideoChannel = (VideoChannel) getIntent().getSerializableExtra(getResources().getString(R.string.video_channel));
    }

    private class SetupPlaylistTask extends AsyncTask<Void, Void, Void> {

        @Override
        protected Void doInBackground(Void... params) {
            String oldLink = mSelectedVideoChannel.getVideoUrl();
            mSelectedVideoChannel.setVideoUrl(PlayListHelper.getPlaylist(oldLink));
            return null;
        }
...
//            showDialog("Downloaded " + result + " bytes");
            startVideoPlayerAsync();
        }
    }

    private void startVideoPlayerAsync() {
//        Bundle b = getIntent().getExtras();
//        mSelectedVideoChannel = (VideoChannel) getIntent().getSerializableExtra(getResources().getString(R.string.video_channel));
//        if (null != b) {
...
//            int startPosition = b.getInt(getResources().getString(R.string.start_position), 0);
        ...
        int startPosition = 0;
        mVideoView.setVideoPath(mSelectedVideoChannel.getVideoUrl());
        Log.d(TAG, mSelectedVideoChannel.getVideoUrl());
    ...
    private class BackToDetailTask extends TimerTask {
        ...
        public void run() {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Intent intent = new Intent(TvbPlayerActivity.this, DetailsActivity.class);
                    intent.putExtra(getResources().getString(R.string.video_channel), mSelectedVideoChannel);
